...
exports.createResource = async (req, res) => {
  try {
    ...
    if (!Name) {
      return res
        .status(400)
        .json({ success: false, message: "El nombre del recurso es requerido" });
    }
    if (!req.user) {
    ...
    if (!isAdmin(req) && !isTherapist(req)) {
      return res
        .status(403)
        .json({ success: false, message: "No tienes permisos para crear recursos." });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error al crear recurso", error: error.message });
  }
};
...
exports.updateResource = async (req, res) => {
  try {
    ...
    // ✅ ADMIN puede siempre
    // ✅ THERAPIST solo si lo publicó
    if (isAdmin(req)) {
      // ok
    } else if (isTherapist(req)) {
      const ok = await resourcePublishedByTherapist(id, req.user.id);
      if (!ok) {
        return res.status(403).json({
          success: false,
          message: "No puedes modificar un recurso que no has publicado.",
        });
      }
    } else {
      return res
        .status(403)
        .json({ success: false, message: "No tienes permisos para modificar recursos." });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Recurso actualizado exitosamente", data: resource });
  } catch (error) {
  ...
  try {
    ...
    if (isAdmin(req)) {
      ...
      if (!ok) {
        return res.status(403).json({
          success: false,
          message: "No puedes eliminar un recurso que no has publicado.",
        });
      }
      ...
      return res
        .status(403)
        .json({ success: false, message: "No tienes permisos para eliminar recursos." });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error al eliminar recurso", error: error.message });
  }
};
...
exports.getResourceConsumers = async (req, res) => {
  try {
    ...
    if (!isAdmin(req) && !isTherapist(req)) {
      return res.status(403).json({
        success: false,
        message: "No tienes permisos para ver consumidores de un recurso.",
      });
    }
      ...
      attributes: [
        ...
        [
          db.sequelize.literal(
            `(SELECT COUNT(*) FROM CONSUME c WHERE c.Id_resource = Resource.Id_resource)`
          ),
          "consumption_count",
        ],
      ],
...
exports.getResourcesStats = async (req, res) => {
  try {
    ...
    if (!isAdmin(req)) {
      return res.status(403).json({ success: false, message: "Solo ADMIN puede ver estadísticas." });
    }

    const stats = await Resource.findAll({
      attributes: ["Resource_type", [db.sequelize.fn("COUNT", db.sequelize.col("Id_resource")), "count"]],
      group: ["Resource_type"],
    });
...
exports.getMyResources = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Error al obtener mis recursos",
      error: error.message,
    });
  }
};
...
exports.getPatientFeed = async (req, res) => {
  try {
    if (!req.user) return res.status(401).json({ success: false, message: "Necesitas iniciar sesión." });
    if (!isPatient(req)) return res.status(403).json({ success: false, message: "Solo pacientes." });
